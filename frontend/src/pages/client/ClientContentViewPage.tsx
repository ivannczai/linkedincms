...
const ClientContentViewPage: React.FC = () => {
  ...
  const fetchContent = useCallback(async () => {
    ...
    try {
      ...
      if (user?.role !== 'client' || data.client_id !== user?.client_id) {
         ...
         try {
           const strategyData = await strategyService.getByClientId(data.client_id);
  ...
  useEffect(() => {
    ...
    const fetchPendingIds = async () => {
      ...
      try {
        const pending = await contentService.getAll(user.client_id, ContentStatus.PENDING_APPROVAL);
  ...
  const submitApprovalAndRating = async (rating: number) => {
     ...
     try {
        ...
        setError(err.message || 'Failed to approve or rate content. Please try again.');
        throw err;
     } finally {
  ...
  const handleMarkAsPosted = async () => {
    ...
    setContent(prev => prev ? { ...prev, status: ContentStatus.PUBLISHED, published_at: new Date().toISOString() } : null);

    try {
      await contentService.markAsPosted(content.id);
      await fetchContent(); // Refetch to confirm
    } catch (err) {
      console.error('Failed to mark content as posted:', err);
      setError('Failed to mark content as posted. Please try again.');
      setContent(originalContent); // Revert optimistic update
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const canMarkAsPosted = content.status === ContentStatus.APPROVED && !content.published_at;
      ...
      {canTakeAction && (
        <div className="sticky top-[calc(4rem+1px)] z-10 bg-white shadow-md mb-6 rounded-b-lg border-t border-gray-200">
          <div className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-3">
             <h2 className="text-lg font-medium text-gray-900 mb-2 md:mb-0 md:inline-block md:mr-4">Actions</h2>
             {error && (
               <div className="mb-2 text-sm bg-red-100 border border-red-400 text-red-700 px-3 py-1 rounded" role="alert">
                 {error}
               </div>
             )}
             <div className="flex flex-wrap gap-2">
               ...
               <button
                 onClick={() => navigate(`/client/contents/${pendingContentIds[currentIndex - 1]}`)}
               ...
               >
                ...
                <button
                 onClick={() => navigate(`/client/contents/${pendingContentIds[currentIndex + 1]}`)}
                 disabled={currentIndex < 0 || currentIndex >= pendingContentIds.length - 1}
        ...
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50 flex items-center justify-center p-4">
          <div className="relative mx-auto border border-gray-200 w-full max-w-md shadow-xl rounded-lg bg-white">
            <div className="flex justify-between items-center border-b p-5">
               <h3 className="text-lg font-semibold text-brand-foreground">Request Revision</h3>
                <button
            ...
            <div className="p-6">
              <label htmlFor="revisionComment" className="form-label mb-1">
                Please provide feedback for revision: <span className="text-red-600">*</span>
              </label>
              <textarea
                id="revisionComment"
                rows={4}
