...
import contentService, { Content, ContentStatus, ContentSortField, SortOrder } from '../../services/contents'; // Import sort types
import { useAuth } from '../../context/AuthContext';
import ContentCard from '../../components/contents/ContentCard';
import { ListFilter } from 'lucide-react'; // Icon for sorting dropdown

// Define sorting options
const sortOptions: { label: string; value: ContentSortField; orders: SortOrder[] }[] = [
...
const ClientContentLibraryPage: React.FC = () => {
  ...
  const sortBy = (searchParams.get('sort_by') || 'created_at') as ContentSortField; // Default sort
   ...
   useEffect(() => {
     const fetchClientContents = async () => {
       ...
       try {
         ...
         const filteredData = data.filter(c => c.status !== ContentStatus.DRAFT);
  ...
  const getPageTitle = () => {
    if (statusFilter === ContentStatus.PENDING_APPROVAL) return 'Content Pending Approval';
    if (statusFilter === ContentStatus.REVISION_REQUESTED) return 'Content Needing Revision';
  ...
  const handleMarkAsPosted = async (id: number) => {
     try {
        await contentService.markAsPosted(id);
        // Refetch content to update the list after marking as posted
        const data = await contentService.getAll(
           user?.client_id,
           statusFilter === '' ? undefined : statusFilter,
           sortBy,
        ...
        console.error("Failed to mark as posted from library:", err);
     }
  };


  if (!user?.client_id) {
     return <div className="p-8 text-red-500">Client information not found.</div>;
  }
  ...
  const currentSortValue = useMemo(() => `${sortBy}:${sortOrder}`, [sortBy, sortOrder]);

  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
        <h1 className="text-2xl font-bold text-brand-foreground">{getPageTitle()}</h1>
         {/* Sorting Dropdown */}
      ...
      {error && <div className="m-6 p-4 bg-red-100 border border-red-400 text-red-700 rounded">{error}</div>}
      {!loading && !error && contents.length === 0 && (
         <div className="text-center py-12">
            <p className="text-gray-500">No content pieces found matching the current filter.</p>
         </div>
      )}
      {!loading && !error && contents.length > 0 && (
