...
const LinkedInSchedulerPage: React.FC = () => {
	...
	const fetchContent = useCallback(async () => {
		...
		try {
			...
			if (user?.role !== 'client' || data.client_id !== user?.client_id) {
				...
				tempDiv.innerHTML = data.content_body || '';
				setContentText(tempDiv.textContent || '');
				setError(null);
			}
		} catch (err) {
	...
	const handleDeletePost = async (postId: number) => {
		...
		try {
			...
			setError(
				err.response?.data?.detail || 'Failed to cancel scheduled post.'
			);
		}
	};
			...
			>
				...
				<div className='mb-4'>
					...
					<p className='text-xs text-gray-500 mt-1'>
						The post will be published at this time.
					</p>
				</div>
				<button
					type='submit'
						...
						{scheduledPosts.map((post) => (
							<li key={post.id} className='px-4 py-4 sm:px-6'>
								<div className='flex items-center justify-between flex-wrap gap-2'>
									<p className='truncate text-sm font-medium text-gray-800 flex-1 min-w-0'>
									...
									<div className='mt-2 flex items-center text-sm text-gray-500 sm:mt-0'>
										...
										{post.status === 'published' && post.linkedin_post_id && (
											<span
												...
												title={`LinkedIn Post ID: ${post.linkedin_post_id}`}
											>
												Published
											</span>
										)}
										...
										{post.status === 'failed' && (
											<p
												...
												title={post.error_message || 'Unknown error'}
											>
												Error:{' '}
												{(post.error_message || 'Unknown error').substring(
													0,
													50
												)}
