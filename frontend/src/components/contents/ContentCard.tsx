...
import { Eye, Edit, Copy, CheckCheck, Star, AlertTriangle, Clock } from 'lucide-react'; // Add Star, AlertTriangle, Clock
...
const ContentCard: React.FC<ContentCardProps> = ({
  ...
  const getCardBorderClass = (status: ContentStatus) => {
    ...
    switch (status) {
      case ContentStatus.PENDING_APPROVAL: return 'border-l-4 border-yellow-400';
      case ContentStatus.REVISION_REQUESTED: return 'border-l-4 border-red-500';
      case ContentStatus.APPROVED: return 'border-l-4 border-green-500';
      default: return 'border-l-4 border-transparent'; // Default transparent border
    }
  };
  ...
  return (
    ...
    <div className={cn("card flex flex-col justify-between h-full", getCardBorderClass(content.status))}>
      <div>
        <div className="flex justify-between items-start mb-2">
          <h3 className="text-lg font-semibold text-brand-foreground mb-1 pr-2 break-words">
            <Link to={viewLink} className="hover:text-primary-600 transition-colors">
              {content.title}
            </Link>
          </h3>
          <span
        ...
        <p className="text-sm text-gray-500 mb-3 line-clamp-2">{getSnippet(content.content_body)}</p>
      </div>
      ...
      <div className="mt-auto pt-4 border-t border-gray-100">
        ...
        <div className="flex space-x-2 justify-end">
          {isClientView && content.status === ContentStatus.PENDING_APPROVAL && (
             <Link to={viewLink} className="btn btn-secondary btn-sm px-3 py-1 text-xs"> {/* Smaller button */}
               <Clock className="h-3 w-3 mr-1" /> Review
             </Link>
          )}
           {isClientView && content.status === ContentStatus.REVISION_REQUESTED && (
             <Link to={viewLink} className="btn btn-danger btn-sm px-3 py-1 text-xs"> {/* Danger style */}
               <AlertTriangle className="h-3 w-3 mr-1" /> Review Feedback
             </Link>
          )}
           {isClientView && content.status === ContentStatus.APPROVED && (
             <>
               <button onClick={handleCopy} className="btn btn-ghost btn-sm px-3 py-1 text-xs">
                  <Copy className="h-3 w-3 mr-1" /> Copy Text
               </button>
               <button onClick={handleMarkPostedClick} className="btn btn-success btn-sm px-3 py-1 text-xs"> {/* Success style */}
                  <CheckCheck className="h-3 w-3 mr-1" /> Mark Posted
               </button>
             </>
           )}
           {!isClientView && ( // Admin actions
              <Link to={editLink} className="btn btn-secondary btn-sm px-3 py-1 text-xs">
                 <Edit className="h-3 w-3 mr-1" /> Edit
              </Link>
           )}
           {/* General View button for non-actionable client items or admin */}
           {(!isClientView || ![ContentStatus.PENDING_APPROVAL, ContentStatus.REVISION_REQUESTED, ContentStatus.APPROVED].includes(content.status)) && (
               <Link to={viewLink} className="btn btn-ghost btn-sm px-3 py-1 text-xs">
